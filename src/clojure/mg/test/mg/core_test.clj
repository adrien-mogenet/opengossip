...
; 4x3 matrix
(def M-2 (matrix [[1 2.21 3.43]
                  [2 3.43 2.32]
                  [3 1.32 4.54]
                  [4 2.32 2.32]]))

(def mu-2    (get (estimate-gaussian M-2) 0))
(def sigma-2 (get (estimate-gaussian M-2) 1))
...
(deftest test-gaussian-estimation
  ...
  (testing "real dataset"
    (let [mu    (matrix [[(/ 13 3) 4.0]])
          sigma (matrix [[(/ 13 3) -1.50] [-1.50 1.00]])]
      (is (=
           [mu sigma]
...
(deftest test-multivariate-normal
  (testing "multivariate-normal"
    (is (=
         [4 1]
         (dim
          (multivariate-normal M-2 mu-2 sigma-2)))))
  (testing "real dataset"
    (let [[mu sigma] (estimate-gaussian feature-matrix)]
      (is (= 3 (nrow (multivariate-normal feature-matrix mu sigma)))))))
